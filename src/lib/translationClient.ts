...
interface TranslationResponse { text: string; fallback: boolean; }
export interface TranslatedResult { data: SimplifiedResult; fallback: boolean; }

const translationCache = new Map<string, TranslatedResult>();

export function getCachedTranslation(target: string, sourceText: string): TranslatedResult | undefined {
  return translationCache.get(target + ":" + sourceText);
}

export function cacheTranslation(target: string, sourceText: string, translated: TranslatedResult): void {
...
async function translateText(text: string, target: string): Promise<TranslationResponse> {
  try {
    ...
    return { text: payload.data ?? text, fallback: !response.ok || payload.fallback === true || !payload.data };
  } catch {
...
export async function translateResult(result: SimplifiedResult, target: string): Promise<TranslatedResult> {
  ...
  const glossary: Glossary = glossaryResults.map((item) => ({ term: item.term, definition: item.definition.text }));

  return {
    data: { ...result, simplifiedText: summary.text, keyPoints: keyPoints.map((item) => item.text), caveats: caveats.map((item) => item.text), glossary },
