export interface SimplifiedResult { simplifiedText: string; keyPoints: string[]; caveats: string[]; glossary?: Array<{ term: string; definition: string }>; }
...
export function heuristicSimplification(documentText: string): SimplifiedResult {
  ...
  return { simplifiedText: sentences.slice(0, 8).join(" ").trim() || "We could not extract enough readable text to simplify this document.", keyPoints: keyPoints.length ? keyPoints : sentences.slice(0, 3).map((sentence) => sentence.trim()), caveats: ["AI simplification is temporarily unavailable; this is a sentence-level fallback. Review the original document carefully."], glossary: [] };
}
...
export function buildChatPrompt(documentText:string,question:string,history:Array<{role:"user"|"assistant";content:string}>):string{return `Answer ONLY from the provided document. If the answer is absent, say exactly: this isn't covered in the document. Do not use outside legal knowledge. This is general information, not legal advice.\nDOCUMENT:\n${documentText}\nHISTORY:\n${history.map(m=>m.role+": "+m.content).join("\n")}\nQUESTION: ${question}`;}