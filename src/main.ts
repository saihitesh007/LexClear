...
wireUpload(document.querySelector<HTMLElement>("#drop-zone")!, document.querySelector<HTMLInputElement>("#document-file")!, document.querySelector<HTMLElement>("#ingestion-status")!, { onText: async (text) => {
  ...
  renderSimplifiedView(results, payload.data);
  mountChatPanel(chatHost, text);
}});
document.querySelector<HTMLButtonElement>("#open-compare")!.onclick = async () => { const view = await import("./views/compareView"); view.mountCompareView(document.querySelector<HTMLElement>("#compare-host")!); };