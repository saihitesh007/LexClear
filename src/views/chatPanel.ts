...
export function mountChatPanel(host: HTMLElement, documentText: string): void {
  ...
  const append = (message: Message) => {
    ...
    item.className = "rounded-xl " + (message.role === "You" ? "bg-slate-800" : "border border-teal-300/20 bg-teal-300/8") + " p-4 text-sm leading-6";
  ...
  host.querySelector<HTMLFormElement>("#chat-form")!.onsubmit = async (event) => {
    ...
    try {
      const response = await fetch("/api/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ documentText, question, history }) });
      ...
      append({ role: "LexClear", content: "Assistant unavailable. Please review the document directly." });
    } finally {
