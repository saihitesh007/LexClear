...
interface TranslatedResult { data: SimplifiedResult; fallback: boolean; }
...
export function renderSimplifiedView(
  ...
  language.onchange = async () => {
    ...
    const cacheKey = target + ":" + original.simplifiedText;
...
export async function translateResult(result: SimplifiedResult, target: string): Promise<TranslatedResult> {
  ...
  return {
    data: {
      ...
      glossary: glossaryResults.map((item) => ({ term: item.term, definition: item.definition.text }))
    },
