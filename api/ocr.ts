...
import { parseDocument, type SupportedMimeType } from "../src/lib/parseDocument";
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  try {
    ...
    if (buffer.byteLength > 10_000_000) { res.status(400).json({ error: "File must be 10 MB or smaller." }); return; }
    const parsed = await parseDocument(buffer, body.mimeType as SupportedMimeType);
    if (parsed.text) { res.status(200).json({ data: { text: parsed.text, ocrUsed: false } }); return; }
