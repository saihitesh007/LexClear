...
describe("Gemini simplification", () => {
  it("builds a grounded structured prompt", () => {
    ...
    expect(prompt).toContain("Preserve every legal obligation, deadline, condition, and monetary figure exactly");
  ...
  it("returns readable heuristic content when Gemini fails", async () => {
    ...
    const result = await simplifyWithFallback("Tenant shall pay $200 within 10 days. The lease renews annually.", invoke);
