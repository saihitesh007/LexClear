...
describe("parseDocument", () => {
  it("extracts clean PDF text", async () => {
    ...
    await expect(parseDocument(Buffer.from("pdf"), "application/pdf", { pdf })).resolves.toEqual({
      text: "Lease terms",
      needsOcr: false,
    });
  });
  it("extracts DOCX text", async () => {
    ...
    ).resolves.toEqual({ text: "Agreement terms", needsOcr: false });
  });
  it("marks empty PDF output for Vision OCR fallback", async () => {
    const pdf = vi.fn().mockResolvedValue({ text: "  " });
    await expect(parseDocument(Buffer.from("scan"), "application/pdf", { pdf })).resolves.toEqual({
      text: "",
      needsOcr: true,
    });
  });
